...
BACKUP_DIR="/opt/unipi-agri-ha/backup"
DOCKER_DIR="/opt/unipi-agri-ha/docker"
...
# バックアップ対象
BACKUP_TARGETS=(
    "/opt/unipi-agri-ha/docker/ha-config"       # Home Assistant設定
    "/opt/unipi-agri-ha/docker/nodered-data"    # Node-REDフロー
    "/opt/unipi-agri-ha/docker/mosquitto/config" # Mosquitto設定
    "/etc/wireguard"                             # WireGuard設定
    "/etc/evok.conf"                             # EVOK設定
    "/opt/unipi-agri-ha/config"                  # カスタム設定
)
