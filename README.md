...
NullClawをデフォルトLLMとして採用。APIキー不要・オフライン動作・無料。
...
| `agriha.vps` | `src/agriha/vps/` | LINE Bot VPS版（systemd + venv） |
...
# NOTE: KEY=VALUE形式のみ。"export" は付けない（systemd EnvironmentFile非対応）
