...
# 2. セットアップ実行（agrihaユーザー作成+venv+systemd）
cd /opt/agriha && sudo bash setup.sh
# 3. 所有権設定
sudo chown -R agriha:agriha /opt/agriha
# 4. 環境変数設定
sudo -u agriha nano /opt/agriha/.env  # ANTHROPIC_API_KEY 等を記入
# 5. サービス起動
sudo systemctl daemon-reload
sudo systemctl enable --now unipi-daemon agriha-ui
```
