...
判断ログからsystem_prompt.txtへの知識蒸留を自動化するパイプライン。
高札（FTS5全文検索）が中核エンジンとなり、LLM呼び出しを段階的に代替する。

```
判断ログ蓄積（control_log.db）
  │
  ▼ 高札インデックス化（SQLite FTS5）
  │
  ▼ forecast_engineが検索で参照（LLM呼び出し前）
  │
  ▼ 検索頻度ログ蓄積（search_log）
  │
  ▼ 頻出パターン抽出（週次バッチ or 反省会トリガー）
  │
  ▼ 反省会で殿に提示（選択式A/B/C、§13）
  │
  ▼ 承認されたらsystem_prompt.txtに蒸留
  │
  ▼ LLM呼び出し頻度が自然減衰（§7.1グラフ）
  │
  ▼ system_prompt.txt自体が「その畑専用の制御教科書」に育つ
```

### 7.6 高札(FTS5)による過去判断検索

forecast_engine（Layer 3）がLLM呼び出し前に高札を検索し、類似判断を参照する。

```
forecast_engine 実行フロー（v3.6追加）:

1. センサーデータ取得（REST API）
2. 【新規】高札に検索クエリを送信
   GET /search?q="内温29℃ CO2_350 湿度72" limit=5
3. 類似判断が見つかった場合:
   a. 過去判断をプロンプトに注入（参考情報として）
   b. 一致度が高い（threshold以上）場合 → LLM呼び出しスキップ
      → 過去判断をそのまま適用（API代ゼロ）
4. 類似判断なし → 従来通りLLM呼び出し
5. LLM判断結果を高札に登録（/reports POST）
```

| 項目 | 仕様 |
|------|------|
| 検索エンジン | SQLite FTS5（既存高札APIの拡張 or 独立インスタンス） |
| 検索対象 | control_log.db の判断ログ + 承認済みルール |
| スキップ閾値 | 類似度スコア ≥ 0.85（設定値、調整可） |
| フォールバック | 検索失敗時は従来通りLLM呼び出し |
| 高札API | `http://localhost:8080/search?q=...`（既存高札の拡張） |

### 7.7 検索ログの蓄積

高札への検索クエリと結果をsearch_logに記録する。
「どんなクエリが何回叩かれたか」が可視化され、頻出パターン抽出の入力となる。

```json
// /var/lib/agriha/search_log.jsonl（1行1JSON、追記型）
{
  "timestamp": "2026-05-15T10:00:00+09:00",
  "query": "内温29℃ CO2_350 湿度72 側窓全開",
  "results_count": 3,
  "top_score": 0.91,
  "skipped_llm": true,
  "applied_judgment": "現状維持（換気中のCO2は自然値で可）"
}
```

### 7.8 頻出パターンの自動検出→ルール昇格

検索ログを週次で集計し、高頻度パターンをルール候補として抽出する。

**昇格の判定基準**:

| 条件 | 内容 |
|------|------|
| 検索回数 | 同一パターンが N 回以上（デフォルト: 10回/週） |
| 一致度 | 類似度スコア平均 ≥ 0.85 |
| 承認状態 | 未ルール化（既存system_prompt.txtに含まれていない） |

**昇格フロー**:

```
高頻度パターン検出（例: 「日射800超+気温28超→側窓全開」が200回）
  │
  ▼ ルール候補として /var/lib/agriha/retrospective/ に保存
  │
  ▼ 反省会（§13）で殿に提示
     Q: 「晴天・高温時は自動で全窓開放してよいですか？」
     A: [A] はい、常に  [B] 条件を絞る  [C] 却下
  │
  ▼ A承認 → system_prompt.txtのルールセクションに追記
  ▼ B承認 → 条件を絞った上で追記（LLMが条件を提案）
  ▼ C却下 → 30日後に自動削除（腐敗設計）
```

### 7.9 system_prompt.txtの進化サイクル

system_prompt.txtは静的ファイルではなく、農家の経験が蒸留される**生きた教科書**である。

```
初期 system_prompt.txt:
  [D] 制御ルール: 汎用的な農業常識（5〜10行）

  ↓ 半シーズン運用後

中期 system_prompt.txt:
  [D] 制御ルール: 汎用ルール + 蒸留ルール（20〜30行）
  例: 「晴天・気温28℃超・側窓全開 → 現状維持（CO2は換気中に自然回復）」

  ↓ フルシーズン運用後

成熟 system_prompt.txt:
  [D] 制御ルール: 畑専用ルール集（50〜100行）
  → LLMは「このルールに該当しない例外」のみ判断
  → API呼び出し: 月数回
```

### 7.10 高札の実装方針

既存高札API（http://localhost:8080）の拡張、または独立インスタンスとして実装する。

| 方針 | メリット | デメリット |
|------|---------|----------|
| **既存高札を拡張**（推奨） | 追加インフラ不要・実績あり | 高札の役割が複合化 |
| 独立インスタンス | 責務分離・スケール容易 | Dockerポート管理が増える |

**推奨実装（既存高札拡張）**:

```
既存高札 (http://localhost:8080):
  /reports    POST  → report登録（既存）
  /reports/N  GET   → report取得（既存）
  /search     GET   → FTS5検索（新規追加）
               ?q=クエリ文字列
               &limit=件数（default: 5）
               &threshold=類似度閾値（default: 0.7）
```

FTS5インデックス対象: `summary` + `body` フィールド（既存reportテーブル）

### 7.11 新規就農者への展開

成熟したsystem_prompt.txtは、同作物・同地域の新規ハウスへの初期値として配布できる。

```
畑Aの成熟 system_prompt.txt
  ↓ 地域・品種が近い場合
畑Bの初期 system_prompt.txt として採用
  ↓ 畑Bで独自の経験を蓄積
畑B専用 system_prompt.txt に進化
```
...
| レベル | 内容 |
|--------|------|
| 汎用版 | リポジトリ初期値（config/system_prompt.txt） |
| 地域版 | 同地域・同作物の農家間共有（GitHubで公開可） |
| 農家専用版 | その畑・その農家専用（/etc/agriha/system_prompt.txt） |

> 汎用AIではなく「その畑専用AI」の正体はsystem_prompt.txt。
> 畑ごと・品種ごと・農家ごとに異なるsystem_prompt.txtが育つ。
...
2026-03-04に§7（LLM自然減衰モデル）に蒸留パイプライン（§7.5〜§7.11）を追加した（v3.6）。
...
| **v3.6** | **蒸留パイプライン（§7.5〜§7.11新設）** | **高札FTS5検索+検索ログ+頻出パターン自動検出+ルール昇格+system_prompt進化サイクル+高札実装方針+新規就農者展開モデル** |
