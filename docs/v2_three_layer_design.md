...
def get_effective_target(pid_override_path):
    ...
    try:
        ...
        if datetime.now(_JST) < valid_until:
            targets = override.get("pid_targets", {})
            return targets.get("temp_setpoint"), override.get("pid_gains_override")
...
rule_engine.py がトリガー条件を検知
  ...
  │   └─ バリデーション:
  │       ✓ pid_targets.temp_setpoint ∈ [16, 30]（範囲外 → スキップ、ログ警告）
...
```json
{
  ...
  "reason": "CO2 280ppm。密閉傾向のため換気強化。目標気温を0.5℃上げて窓開を促進。",
  "confidence": "high"
}

注意:
- pid_targets.temp_setpoint は必須。pid_gains_override は省略可（省略時はPIDデフォルトゲインを使用）
...
# === Starlink監視（10分毎, §14: Connectivity Monitor）===
# offline判定: 3回連続失敗(30分) → Layer3スキップ → Layer1+2継続
*/10 * * * * root ping -c 3 8.8.8.8 \
  && echo "{\"status\":\"online\",\"ts\":\"$(date -Iseconds)\"}" > /var/lib/agriha/starlink_status.json \
  || echo "{\"status\":\"offline\",\"ts\":\"$(date -Iseconds)\"}" >> /var/lib/agriha/connectivity.log

# === 計画アーカイブ（毎日23:55, §13: Weekly Retrospective）===
# 当日計画を plans/{date}_plan.json としてアーカイブ
55 23 * * * root cp /var/lib/agriha/current_plan.json \
  /var/lib/agriha/plans/$(date +\%Y\%m\%d)_plan.json 2>/dev/null || true
```
...
### 5.6 WebUI計画グラフ表示との接続点（§13: Weekly Retrospective）

WebUI（agriha_ui, port 8502）が反省会の計画vs実績グラフを表示するための接続点。

| データ | 生成元 | 参照先 | 形式 |
|--------|--------|--------|------|
| `current_plan.json` | forecast_engine.py | WebUI GET /api/plan | pid_override スキーマ準拠 |
| `{date}_plan.json` | cron 23:55 アーカイブ（§4） | WebUI GET /api/plan/history | 日次アーカイブ |
| `{date}_actual.json` | rule_engine.py 実行結果 | WebUI GET /api/actual/history | 日次実績記録 |
| `pid_state.json` | rule_engine.py | WebUI GET /api/pid/state | PID積分項・現在目標値 |

```
WebUI (port 8502)
  └─ GET /api/plan/history?date=YYYYMMDD
       → /var/lib/agriha/plans/{date}_plan.json を読み込み
  └─ GET /api/actual/history?date=YYYYMMDD
       → /var/lib/agriha/plans/{date}_actual.json を読み込み
  └─ タイムラインChart.js でグラフ表示（計画(青)vs実績(橙)の重ね表示）
```
...
### 6.6 /var/lib/agriha/retrospective/config.yaml（v3.5新設）

反省会モード（§13）の設定ファイル。

```yaml
# /var/lib/agriha/retrospective/config.yaml
# Weekly Retrospective 設定（§13: llm_control_loop_design.md v3.5）

retrospective:
  frequency: weekly          # weekly / monthly / daily
  day: monday                # 送信曜日（weeklyの場合）
  nudge_if_no_response: 2    # N週無反応でリマインド送信
  rule_expiry_days: 30       # 未承認ルール候補の自動削除日数（腐敗設計）
...
```bash
# /etc/agriha/starlink_monitor.env
# Starlink監視設定（§14: llm_control_loop_design.md v3.5）

PING_TARGET="8.8.8.8"          # または 1.1.1.1
OFFLINE_THRESHOLD=3             # N回連続失敗でoffline判定（30分）
STATUS_FILE="/var/lib/agriha/starlink_status.json"
CONNECTIVITY_LOG="/var/lib/agriha/connectivity.log"
```
...
/var/lib/agriha/                      # ランタイムデータ（read-write）
  ...
  └─ plans/                           # 新規(v1.5): 計画/実績アーカイブ（反省会用）
       ...
       └─ {YYYYMMDD}_actual.json      # 日次実績（rule_engine.py記録）

/var/lib/agriha/retrospective/        # 反省会データ（§13）
  ├─ config.yaml                      # 新規(v1.5): 反省会設定（§6.6）
  └─ {YYYYMMDD}_retro.json            # 週次振り返り記録（ルール候補+殿の回答）
...
/var/log/agriha/                      # ログファイル
  ...
  └─ agriha-capture.log              # 新規(v1.5): CSIカメラ撮影ログ
