...
agriha_control.py の判断履歴 API は RPi 上の agriha-chat で動作する。
WireGuard VPN 経由で RPi に接続する（デフォルト: 10.10.0.10:8501）。

環境変数:
  RPI_API_URL     RPi の REST API ベース URL (例: http://10.10.0.10:8080)
  RPI_API_KEY     API キー (空文字で認証スキップ)
  RPI_API_TIMEOUT タイムアウト秒数 (デフォルト: 10)
  CHAT_API_URL    判断履歴 API のベース URL (例: http://10.10.0.10:8501)
"""
...
CHAT_API_URL = os.getenv("CHAT_API_URL", "http://10.10.0.10:8501")
...
def get_history(limit: int = 10) -> list[dict]:
    """GET /api/history — agriha_control.py の直近制御判断履歴を取得する。

    Args:
        limit: 取得件数 (デフォルト: 10)

    Returns:
        [{"timestamp": str, "summary": str, "actions_taken": str, "sensor_snapshot": str}, ...]
        エラー時: [] (空リスト。履歴取得失敗でも LINE 応答は継続)
    """
    try:
        with httpx.Client(timeout=TIMEOUT_SEC) as client:
            resp = client.get(
                f"{CHAT_API_URL}/api/history",
                params={"limit": limit},
            )
            resp.raise_for_status()
            return resp.json()
    except httpx.ConnectError:
        logger.warning("判断履歴 API 接続エラー: %s", CHAT_API_URL)
        return []
    except httpx.TimeoutException:
        logger.warning("判断履歴 API タイムアウト")
        return []
    except httpx.HTTPStatusError as exc:
        logger.error("判断履歴 API HTTP エラー %d", exc.response.status_code)
        return []
    except Exception as exc:
        logger.error("判断履歴 API エラー: %s", exc)
        return []
