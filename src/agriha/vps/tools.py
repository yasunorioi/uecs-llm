...
Claude tool use (Anthropic Messages API) 用のスキーマと、
ツール実行ロジックを提供する。
...
ツール一覧:
  ...
  control_history   agriha_control.py の制御判断履歴取得
"""
...
from rpi_client import get_history, get_sensors, set_relay

logger = logging.getLogger(__name__)

# ---------------------------------------------------------------------------
# ツールスキーマ定義 (Claude tool use / Anthropic Messages API 互換 JSON Schema)
# ---------------------------------------------------------------------------
...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "直近の環境制御判断履歴を取得する。"
                "「なぜ窓を開けたの？」「最後の灌水はいつ？」「今日の制御は？」などに使う。"
                "agriha_control.py（5分ごとのcron制御ループ）が行った判断の記録を返す。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "hours": {
                        "type": "integer",
                        "description": "何時間前まで遡るか（デフォルト: 24）",
                        "default": 24,
                        "minimum": 1,
                        "maximum": 168,
                    },
...
def execute_tool_call(tool_call: dict[str, Any]) -> str:
    """Claude ツールコールを実行し、結果を JSON 文字列で返す。

    Args:
        tool_call: tool_calls の 1 エントリ
    ...
    if name == "sensor_status":
        ...
        # limit は hours×12回（5分ごと）を上限とする
        limit = min(hours * 12, 200)
